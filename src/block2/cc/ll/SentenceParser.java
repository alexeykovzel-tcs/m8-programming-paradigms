...
public class SentenceParser implements Parser {
	...
	private AST parseSentence() throws ParseException {
		...
		result.addChild(parseToken(Sentence.VERB));
		result.addChild(parseObject());
		result.addChild(parseToken(Sentence.ENDMARK));
		return result;
	}
	...
	private AST parseSubject() throws ParseException {
		...
		switch (next.getType()) {
		case Sentence.ADJECTIVE:
			result.addChild(parseModifier());
			result.addChild(parseSubject());
			break;
		case Sentence.NOUN:
			result.addChild(parseToken(Sentence.NOUN));
			break;
		default:
	...
	private AST parseModifier() throws ParseException {
		...
		result.addChild(parseToken(Sentence.ADJECTIVE));
		return result;
	}
