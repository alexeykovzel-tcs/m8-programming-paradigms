...
import block4.cc.iloc.asm.parse.ErrorListener;
import block4.cc.iloc.asm.parse.FormatException;
import block4.cc.iloc.asm.parse.ILOCBaseListener;
import block4.cc.iloc.asm.parse.ILOCLexer;
import block4.cc.iloc.asm.parse.ILOCParser;
import block4.cc.iloc.asm.parse.ILOCParser.*;
...
public class Assembler {
	...
	private static class ILOCWalker extends ILOCBaseListener {
		...
		private List<Operand> getOpnds(OpCode code, String role,
                                       List<Operand.Type> sig, List<Operand> orig) throws FormatException {
			...
			for (int i = 0; i < orig.size(); i++) {
				...
				Operand.Type actualType = opnd.getType();
				Operand.Type expectedType = sig.get(i);
				if (expectedType == Operand.Type.REG && actualType == Operand.Type.LABEL) {
